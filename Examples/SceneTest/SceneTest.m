...
-(id) init
{
	if( (self=[super initWithColor: ccc4(0,0,255,255)]) ) {
		...
		[self addChild:label];
		CGSize s = [[CCDirector sharedDirector] winSize];
		[(CCNode *)label setPosition:ccp(s.width/2, s.height/2)];
...
- (void) applicationDidFinishLaunching:(UIApplication*)application
{
    ...
    // Must do manually after the final orientation is set
    ...
    [self performSelector:@selector(stopRecordingAndShowDialog:) withObject:nil afterDelay:5.0];
